...
//   const [groupedCrimes, setGroupedCrimes] = React.useState < any > null;

//   React.useEffect(() => {
//     const fetchData = async () => {
//       const res = await fetchGroupedByMonth();
//       setGroupedCrimes(res.data);
//     };
//     fetchData();
//   }, [fetchGroupedByMonth]);
...
export function CrimePieChart() {
  const { fetchGroupedByMonth, loading } = useCrime();
  const [activeMonth, setActiveMonth] = React.useState('January');
  const [groupedCrimes, setGroupedCrimes] = React.useState(null);
  ...
  const crimeColorMap = {
    ...
    FRAUD: 'var(--chart-2)',
    ASSAULT: 'var(--chart-3)',
    BURGLARY: 'var(--chart-4)',
    OTHER: 'var(--chart-5)',
  };

  const chartDataFromAPI = React.useMemo(() => {
    if (!groupedCrimes || !groupedCrimes[activeMonth]) return [];

    return Object.entries(groupedCrimes[activeMonth].crimes).map(
      ([crimeName, count]) => ({
        crimeName,
        value: count,
        fill: `hsl(${crimeColorMap[crimeName] || 'var(--chart-5)'})`,
      })
    );
  }, [groupedCrimes, activeMonth]);
  // const chartDataFromAPI = React.useMemo(() => {
  //   if (!groupedCrimes || !groupedCrimes[activeMonth]) return [];
  ...
  //       crimeName,
  //       value: count,
  //       fill: `hsl(var(--chart-${crimeName.toLowerCase()}))`,
  //     })
  //   );
  // }, [groupedCrimes, activeMonth]);

  const totalCrimes = React.useMemo(() => {
    if (!groupedCrimes || !groupedCrimes[activeMonth]) return 0;
    return groupedCrimes[activeMonth].total;
  }, [groupedCrimes, activeMonth]);

  return (
    <Card className="w-full max-w-[100%] h-[470px]">
      <CardHeader className="items-center pb-2 flex">
        <CardTitle className="text-base">Donut Pie Chart - Crimes</CardTitle>{' '}
        <CardDescription className="text-sm">
          January - June 2024
        </CardDescription>
        <Select value={activeMonth} onValueChange={setActiveMonth}>
          ...
          <SelectContent align="end" className="rounded-xl">
            {groupedCrimes &&
              Object.keys(groupedCrimes).map((month) => (
                <SelectItem
                  key={month}
                  value={month}
                  className="rounded-lg [&_span]:flex"
      ...
      <CardContent className="flex-1 pb-2">
        {chartDataFromAPI.length > 0 && (
          // <ChartContainer
          //   config={chartConfig}
          //   className="mx-auto aspect-square max-h-[200px] h-[300px] w-[300px]"
          // >
          <ChartContainer
            config={chartConfig}
            className="mx-auto aspect-square w-full max-w-[220px] sm:max-w-[240px]"
          >
            <PieChart>
              <ChartTooltip
                cursor={false}
              ...
              <Pie
                ...
                strokeWidth={4}
              >
                {chartDataFromAPI.map((entry, idx) => (
